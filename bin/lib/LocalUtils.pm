...
sub draw_color_legend {
	my ($svg, $LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, $legend_type,$shapeType_ref) = @_;
	...
	if  ($layout ==1  || $layout ==3 ||  $layout ==5 )
	{
		...
		if ($layout ==3)
		{
		  $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_Height"}-$HashConfi_ref->{"global"}{"canvas_margin_bottom"}+$ColorBarSize*0.68+$HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
		...
		elsif ($layout ==5)
		{
		   ...
		   $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} + $HashConfi_ref->{$Level}{"colormap_legend_shift_y"}+$Legend_Count*$MaxGradien*$ColorBarSize;
		...
		{
			my $YY2=$YY1+$ColorBarSize*($max_index_legend+1);
		...
		foreach my $k (0 ..$max_index_legend) {
			...
			my $current_yy1 = $YY1 + $ColorBarSize * $k;
			...
			if ($legend_type eq 'rectangle') {
				...
				ColorPaletteManager::SVGgetShape($XX1+$ColorBarSize * 0.5, $current_yy1 + $ColorBarSize * 0.5, $ColorBarSize * 0.5, $shapeType_ref->[$k], $ColorGradientArray_ref->[$k], $svg);
			}
	...
	elsif ($layout ==2 || $layout ==4 )
	{
		my $YY2 = $HashConfi_ref->{"global"}{"canvas_margin_Height"}-$ColorBarSize*1.5+$HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
		my $MaxGradien=$HashConfi_ref->{"global"}{"MaxGradien"}+1;

		my $XX1 = $HashConfi_ref->{"global"}{"canvas_margin_left"} +$Legend_Count* $ColorBarSize * $MaxGradien+1.5+$HashConfi_ref->{$Level}{"colormap_legend_shift_x"};
		...
		foreach my $k (0 ..$max_index_legend) {
			next if (!defined($ValueLabelsGradient_ref->[$k]));
			my $current_XX1 = $XX1 + $ColorBarSize * $k;
			...
			if ($legend_type eq 'rectangle') {
				...
				ColorPaletteManager::SVGgetShape($current_XX1+$ColorBarSize * 0.5, $YY2 + $ColorBarSize * 0.5, $ColorBarSize * 0.5, $shapeType_ref->[$k], $ColorGradientArray_ref->[$k], $svg);
			}
			elsif ($legend_type eq 'lines') {
...
sub plot_heatmap {
	...
	draw_color_legend($svg,$LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'rectangle');
...
sub plot_point {
	...
	if ($NumPlotArry == 1) {
		...
		draw_color_legend($svg,$LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'circle');
...
sub plot_shape {
	...
	if ($NumPlotArry == 1) {
		...
		draw_color_legend($svg,$LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'shape',\@shapeType);
...
sub plot_text {
	...
	draw_color_legend($svg,$LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'text-only');
...
sub plot_line {
	...
	draw_color_legend($svg,$LegendOffsetRatio, $Bodyheight, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'lines');
