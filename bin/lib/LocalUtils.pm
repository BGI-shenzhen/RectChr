...
sub draw_color_legend {
	my ($svg, $XX1, $YY1_start, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, $legend_type) = @_;
	$XX1+=$ColorBarSize+1;
	...
	{
		my $YY2=$YY1_start+$ColorBarSize*($max_index_legend+1);
		my $path = $svg->get_path(
			x => [$XX1, $XX1, $XX2,$XX2],
			y => [$YY1_start, $YY2, $YY2,$YY1_start],
			-type => 'polygon');
	...
	foreach my $k (0 ..$max_index_legend) {
		...
		my $current_yy1 = $YY1_start + $ColorBarSize * $k;
		...
		if ($legend_type eq 'rectangle') {
			...
			$svg->text(
				'text-anchor', 'start',
				'x', $XX1 +1.5, 
				#length($ValueLabelsGradient_ref->[$k]) + $ColorBarSize * 1.88,
				'y', $current_yy2,
...
sub plot_heatmap {
	my ($HashConfi_ref,$ValueToColor_ref, $Level, $svg, $hashYY1_ref, $hashYY2_ref, $hashXX1_ref, $ChrMax, $NumPlotArry, $PlotInfo_ref, $FileData_ref, $FileRow_ref, $ValueToCustomColor_ref, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $ColorBarSize, $all_chromosomes_spacing, $Bodyheight,$LegendOffsetRatio ,$GradientSteps,$fontsize) = @_;
	...
	my $XX1 = $HashConfi_ref->{"global"}{"canvas_margin_left"} + $HashConfi_ref->{"global"}{"canvas_body"} + ($Level - 1) * $ColorBarSize * 4.5 + $HashConfi_ref->{$Level}{"colormap_legend_shift_x"} + $all_chromosomes_spacing;
	my $XX2 = $XX1 + $ColorBarSize;
	if  ($NumPlotArry>1)
	{
		@$ColorGradientArray_ref = @$ColorGradientArray_ref[0..($NumPlotArry - 1)];
	}
	draw_color_legend($svg, $XX1, $YY1, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'rectangle');
	...
	foreach my $tmpkk (1..$NumPlotArry) {
		...
		for (; $StartCount < $FileRow_ref->[$FileNow]; $StartCount++) {
			...
			my $XX1 = sprintf("%.1f", ($StartSite / $ChrMax) * $HashConfi_ref->{"global"}{"canvas_body"} + $hashXX1_ref->{$ThisChrName}{$Level});
			my $XX2 = sprintf("%.1f", ($EndSite / $ChrMax) * $HashConfi_ref->{"global"}{"canvas_body"} + $hashXX1_ref->{$ThisChrName}{$Level});
...
sub plot_link_self {
	my ($HashConfi_ref,$ValueToColor_ref, $Level, $svg, $hashYY1_ref, $hashYY2_ref, $hashXX1_ref,$ChrMax,$NumPlotArry, 
		...
		$all_chromosomes_spacing, $Bodyheight, $LegendOffsetRatio, $GradientSteps,$fontsize)=@_;
	...
	else
	{
		...
		for ( ; $StartCount<$FileRow_ref->[$FileNow]; $StartCount++)
		{
			...
			my $YY1_AA=$YY1;
			my $YY2_AA=$YY1;
			my $XStart_AA=sprintf ("%.1f",($StartA/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$hashXX1_ref->{$ThisChrName}{$Level});
			my $XEnd_AA=sprintf ("%.1f",($EndA/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$hashXX1_ref->{$ThisChrName}{$Level});

			my $YY1_BB=$YY2;
			my $YY2_BB=$YY2;
			my $XStart_BB=sprintf ("%.1f",($StartB/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$hashXX1_ref->{$chrB}{$Level});
			my $XEnd_BB=sprintf ("%.1f",($EndB/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$hashXX1_ref->{$chrB}{$Level});
...
sub plot_pairwiselinkV2 {
	my (
		...
		$ColorBarSize,       # 图例大小比例
		$all_chromosomes_spacing, # 所有染色体间距
		$Bodyheight,        # 主体高度
	...
	if ( (exists ($HashConfi_ref->{$Level}{"link_linestyle"}))   &&  ( $HashConfi_ref->{$Level}{"link_linestyle"} eq "line") )
	{
		if ((exists ($HashConfi_ref->{$Level}{"link_direction"}))   &&  ($HashConfi_ref->{$Level}{"link_direction"} eq "DownUp" ))
		{
			for ( ; $StartCount<$FileRow_ref->[$FileNow]; $StartCount++ )
			{
				...
				my $XX1=sprintf ("%.1f",($StartSite/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$$hashXX1_ref{$ThisChrName}{$Level});
				my $XX2=sprintf ("%.1f",($EndSite/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$$hashXX1_ref{$ThisChrNameBB}{$Level});
				if (exists $ValueToCustomColor_ref->{$Value} ) {$ValueToColor_ref->{$Value}=$ValueToCustomColor_ref->{$Value};}
...
sub plot_pairwiselink {
	my ($HashConfi_ref, $ValueToColor_ref, $Level, $svg, $hashYY1_ref, $hashYY2_ref, $hashXX1_ref,
		...
		$ValueLabelsGradient_ref, $ColorBarSize, $all_chromosomes_spacing, $Bodyheight, $LegendOffsetRatio, $GradientSteps, $fontsize) = @_;
...
sub plot_histogram {
	my ($HashConfi_ref, $ValueToColor_ref, $Level, $svg, $hashYY1_ref, $hashYY2_ref, $hashXX1_ref,
		...
		$ColorGradientArray_ref, $ValueLabelsGradient_ref, $ColorBarSize, $all_chromosomes_spacing, $Bodyheight, 
...
sub plot_point {
	...
	if ($NumPlotArry == 1) {
		my $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} + $LegendOffsetRatio * $Bodyheight + $HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
		my $YY2 = $YY1 + $ColorBarSize * ($GradientSteps + 1);
		my $XX1 = $HashConfi_ref->{"global"}{"canvas_margin_left"} + $HashConfi_ref->{"global"}{"canvas_body"} + ($Level - 1) * $ColorBarSize * 4.8 + $HashConfi_ref->{$Level}{"colormap_legend_shift_x"} + $all_chromosomes_spacing;
		my $XX2 = $XX1 + $ColorBarSize;

		draw_color_legend($svg, $XX1, $YY1, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'circle');
		...
		if (!exists $HashConfi_ref->{$Level}{"colormap_brewer_name"}) {
			...
			ColorPaletteManager::GetColGradien($ColFlag, $NumGradien, $ColorGradientArray_ref);
...
sub plot_shape {
	my ($HashConfi_ref, $ValueToColor_ref, $Level, $svg, $hashYY1_ref, $hashYY2_ref, $hashXX1_ref,
		$ChrMax, $NumPlotArry, $PlotInfo, $FileData_ref, $FileRow_ref, $ValueToCustomColor_ref, $ColorGradientArray_ref,
		$ValueLabelsGradient_ref, $ColorBarSize, $all_chromosomes_spacing, $Bodyheight, $LegendOffsetRatio, $GradientSteps,
	...
	if ($NumPlotArry == 1) {
		my $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} + $LegendOffsetRatio * $Bodyheight + $HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
		my $YY2 = $YY1 + $ColorBarSize * ($GradientSteps + 1);
		my $XX1 = $HashConfi_ref->{"global"}{"canvas_margin_left"} + $HashConfi_ref->{"global"}{"canvas_body"} + ($Level - 1) * $ColorBarSize * 4.8 + $HashConfi_ref->{$Level}{"colormap_legend_shift_x"} + $all_chromosomes_spacing+$ColorBarSize;
		my $XX2 = $XX1 + $ColorBarSize;

		my @shapeType = map { $_ % 16 } 0 .. $GradientSteps;
		...
		if ($HashConfi_ref->{$Level}{"colormap_legend_show"} == 0 || $HashConfi_ref->{$Level}{"colormap_legend_sizeratio"} <= 0) {
			...
			foreach my $k (0 .. $GradientSteps) {
				$YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} + $LegendOffsetRatio * $Bodyheight + $ColorBarSize * $k + $HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
				$YY2 = $YY1 + $ColorBarSize;

				ColorPaletteManager::SVGgetShape($XX1, $YY2 - $ColorBarSize * 0.5, $ColorBarSize * 0.5, $shapeType[$k], $ColorGradientArray_ref->[$k], $svg);
				$Col2Shape{$ColorGradientArray_ref->[$k]} = $shapeType[$k];
				$svg->text(
					'text-anchor', 'start',
					'x', $XX2 +1.5,
		...
		my @shapeType = map { $_ % 16 } 0 .. $NumPlotArry - 1;
		if (exists $HashConfi_ref->{$Level}{"track_geom_shape"}) {
			my @ccc = split(/,/, $HashConfi_ref->{$Level}{"track_geom_shape"});
			foreach my $k (0 .. $#ccc) {
				if ($ccc[$k] < 16) {
		...
		foreach my $k (1 .. $NumPlotArry) {
			...
			my $XX1 = $HashConfi_ref->{"global"}{"canvas_margin_left"} + $HashConfi_ref->{"global"}{"canvas_body"} +
			($Level - 1) * $ColorBarSize * 4.5 + $HashConfi_ref->{$Level}{"colormap_legend_shift_x"} + $all_chromosomes_spacing;
			my $XX2 = $XX1 + $ColorBarSize;
			my $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} + $LegendOffsetRatio * $Bodyheight +
			$ColorBarSize * ($k - 1) + $HashConfi_ref->{$Level}{"colormap_legend_shift_y"};
			my $YY2 = $YY1 + $ColorBarSize;

			if ($HashConfi_ref->{$Level}{"colormap_legend_show"} == 0 || $HashConfi_ref->{$Level}{"colormap_legend_sizeratio"} <= 0) {
				# Skip legend drawing
			} else {
				ColorPaletteManager::SVGgetShape($XX1, $YY2 - $ColorBarSize * 0.5, $ColorBarSize * 0.5, $shapeT, $ColorGradientArray_ref->[$ThisBoxbin], $svg);
				$svg->text(
					'text-anchor', 'middle',
					'x', $XX2 + length($ValueLabelsGradient_ref->[$ThisBoxbin]) + $ColorBarSize * 1.88,
					'y', $YY2,
					'-cdata', $ValueLabelsGradient_ref->[$ThisBoxbin],
					'font-family', 'Arial',
					'font-size', $ColorBarSize
				);
			}
...
sub plot_text {
	...
	draw_color_legend($svg, $XX1, $YY1, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'text-only');
	...
	foreach  my $ThisChrName  (sort keys %sorted_data)
	{
		...
		foreach my $StartSite  (sort {$a<=>$b} keys %$Tmp)
		{
			...
			$XX1 = sprintf("%.1f", ($StartSite / $ChrMax) * $HashConfi_ref->{"global"}{"canvas_body"} + $hashXX1_ref->{$ThisChrName}{$Level});
...
sub plot_ridgeline {
	...
	if (!exists $HashConfi_ref->{$Level}{"colormap_brewer_name"}) {
		my @StartRGB = HTML2RGB($HashConfi_ref->{$Level}{"colormap_low_color"});
		my @EndRGB   = HTML2RGB($HashConfi_ref->{$Level}{"colormap_high_color"});
		my @MidRGB   = HTML2RGB($HashConfi_ref->{$Level}{"colormap_mid_color"});
		...
		my $ColFlag = $HashConfi_ref->{$Level}{"colormap_brewer_name"};
		ColorPaletteManager::GetColGradien($ColFlag, $NumPlotArry, $ColorGradientArray_ref);
		if (exists $HashConfi_ref->{$Level}{"colormap_reverse"}) {
	...
	foreach my $tmpkk (1 .. $NumPlotArry) {
		...
		if ($HashConfi_ref->{$Level}{"colormap_legend_show"} == 0 || $HashConfi_ref->{$Level}{"colormap_legend_sizeratio"} <= 0) {
			...
			$svg->line(
				...
				'stroke-width', $HashConfi_ref->{"ALL"}{"stroke-width"}
			);
			$svg->text(
				'text-anchor', 'middle',
				'x', $XX2 + length($ValueLabelsGradient_ref->[$ThisBoxbin]) + $ColorBarSize * 1.88,
...
sub draw_cutoff_line {
	...
	foreach my $thisChr (0 .. $#$ChrArry_ref) {
		...
		my $XX2 = sprintf("%.1f", ($hashChr_ref->{$ThisChrName} / $ChrMax) * $HashConfi_ref->{"global"}{"canvas_body"} + $hashXX1_ref->{$ThisChrName}{$Level});
		my $YY1 = $hashYY1_ref->{$ThisChrName}{$Level};
		my $YY2 = $hashYY2_ref->{$ThisChrName}{$Level};
...
sub plot_line {
	...
	my $YY1 = $HashConfi_ref->{"global"}{"canvas_margin_top"} +
	$LegendOffsetRatio * $Bodyheight +
	#$LegendOffsetRatio * $Bodyheight + $ColorBarSize  +
	$HashConfi_ref->{$Level}{"colormap_legend_shift_y"};

	draw_color_legend($svg, $XX1, $YY1, $ColorBarSize, $ColorGradientArray_ref, $ValueLabelsGradient_ref, $HashConfi_ref, $Level, 'lines');
...
sub plot_histogram_animated {
	...
	$XX1=$HashConfi_ref->{"global"}{"canvas_margin_left"}+$HashConfi_ref->{"global"}{"canvas_body"}+($Level-1)*$ColorBarSize*4.5+$HashConfi_ref->{$Level}{"colormap_legend_shift_x"}+$all_chromosomes_spacing;
