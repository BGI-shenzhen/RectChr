...
sub Deal_Para {
	...
	return (\%HashConfi, \%Value2SelfCol, \%hashChr, \%hashChr2File,
		...
		$total_chr_length, $MaxGradien, $RegionStart, $bin,
...
sub init_global_defaults {
	return (
		global => {
			...
			canvas_margin_bottom => 25,
			canvas_margin_left   => 100,
			canvas_margin_right  => 120,
			strokewidth          => 1,
			'stroke-width'       => 1,
			canvas_angle         => 0,
...
sub adjust_value_x_based_on_max_level {
	...
	if ($MaxLevel > $NumberLevel && !$TF) {
		log_message('warn', "The max Level found is $MaxLevel, but global.ValueX = $NumberLevel. Auto updated.");
...
sub warn_if_all_level_conflict {
	...
	if (exists $HashConfi->{ALL}{colormap_low_color} ||
		...
		log_message('warn', "Warning:\tSetParaFor= [ALL], Para (crBegin/crMid/crEnd/ColorBrewer) should be seted in each Level,move it to  SetParaFor=LevelX");
	}
}
...
sub validate_and_fix_parameters {
	...
	if (exists $para_ref->{track_height} && $para_ref->{track_height} < 5) {
		log_message('warn', "Level:$Level Para [ChrWidth] too small, modified to 5");
	...
	if ($para_ref->{colormap_nlevels} > 255) {
		log_message('warn', "Level:$Level Para [-Gradien] is Max Gradien must < 255; we modify $$para_ref{Gradien} ---> 255");
		$para_ref->{colormap_nlevels} = 255;
	} elsif ($para_ref->{colormap_nlevels} < 3) {
		log_message('warn', "Level:$Level Para [-Gradien] is seted to be $$para_ref{Gradien}, but too small, we modify it to be 3");
		...
		foreach my $tmpA (@temAA) {
			...
			if ($temBB[0] =~ s/File//g) {
				...
				foreach my $coumn (@temCC) {
					$coumn--;
					if ($coumn < 3 || $coumn > $VV) {
						log_message('error', "File " . ($temBB[0] + 1) . " Column only $VV+1, but you give the ShowColumn is $coumn");
					} else {
						push @ShowColumnArry, [$temBB[0], $coumn];
					}
				}
			} else {
				log_message('error', "Para [ShowColumn] For $Level Format wrong");
			}
		}
	}
...
sub calculate_dimensions {
	...
	if ($HashConfi->{global}{chr_orientation} ne "vertical") {
		$Bodyheight = 1.5 * ($widthForPerChr + $chr_spacing);
	}
	...
	if ($HashConfi->{ALL}{colormap_legend_show} == 0 || $HashConfi->{ALL}{colormap_legend_sizeratio} <= 0) {
	} else {
		$HashConfi->{global}{canvas_margin_right} += ($NumberLevel) * $color_gradient_scale * 2.4;
	}
...
sub check_colorbrewer_usage {
	...
	if ( exists $para_ref->{colormap_brewer_name})
	{
		...
		unless (exists $MAX_COLOR_COUNT{$color_name}) {
			log_message('warn', "RColor Brewer must be in specified name like BrBG/PiYG/PRGn..., but $color_name is not valid\n Changed $color_name  --->  'GnYlRd'\n See: https://www.r-graph-gallery.com/38-rcolorbrewers-palettes.html\n");
		...
		if (exists $QualColNum{$color_name}) {
			...
			if ($para_ref->{colormap_nlevels} > $max_color_count) {
				log_message('warn', "Level $Level: [-Gradien] exceeds max color count for ColorBrewer '$color_name'. Modified from $para_ref->{colormap_nlevels} to $max_color_count.");
				$para_ref->{colormap_nlevels} = $max_color_count;
			}
		}
	}
