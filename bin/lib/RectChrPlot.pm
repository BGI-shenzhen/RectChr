...
my ($HashConfi_ref, $ValueToCustomColor_ref,$hashChr_ref,$ShowColumn_ref, $FileData_ref, $FileRow_ref,$ChrArry_ref,$widthForPerChr,$ChrMax,$total_chr_length,$MaxGradien,
...
my $XX2=$XX1+$all_chromosomes_spacing+$HashConfi_ref->{"global"}{"stroke-width"};
...
# Axis drawing and chromosome label positioning
my $BinXX=($HashConfi_ref->{"global"}{"canvas_body"}+$all_chromosomes_spacing)/$ScaleNum;
...
foreach my $k (0..$ScaleNum)
{
	...
	$svg->line('x1',$XX1,'y1',$YY1,'x2',$XX1,'y2',$YY4,'stroke','black','stroke-width',$HashConfi_ref->{"global"}{"stroke-width"},'fill',$HashConfi_ref->{"global"}{"fill"}); #X
}
...
foreach my $thisChr (0..$#$ChrArry_ref)
{
	...
	$XX2=($hashChr_ref->{$ThisChrName}/$ChrMax)*$HashConfi_ref->{"global"}{"canvas_body"}+$XX1;
		...
		if (!( ($HashConfi_ref->{$Level}{"background_color"}  eq  "#FFFFFF" )  && ($HashConfi_ref->{$Level}{"bg_stroke_color"} eq  "#FFFFFF") ) && (! (exists $HashConfi_ref->{$Level}{"background_show"}  &&  $HashConfi_ref->{$Level}{"background_show"}==0) ) )
		{
			...
			else
			{
				...
				if (exists $HashConfi_ref->{$Level}{"bg_end_arc_division"})
				{
					...
					{
						print "Track(Level) $Level Para EndvurveRadian must >=2 ,so we chang it to be 2\n";
	...
	{
		$ShiftChrNameX=$ShiftChrNameX-length($ThisChrName)*$AAAfontsize*$chr_name_ratio-$fontsize*0.2;
	}
	my $ShiftChrNameY=$HashConfi_ref->{"ALL"}{"chr_label_shift_y"};

	my $XX3tmp=$XX1-length($ThisChrName)*$AAAfontsize*$chr_name_ratio+$ShiftChrNameX;
	...
	if ( $HashConfi_ref->{"global"}{"chr_orientation"} ne "vertical" )
	{
		 ...
		 $XX3tmp=($XX2-$XX1)*0.5+$XX1-$AAAfontsize*0.5;
	}

	if ($HashConfi_ref->{"global"}{"chr_label_rotation"}==0)
	{
		$svg->text('text-anchor','middle','x',$XX3tmp,'y',$TextYY,'-cdata',$ThisChrName,'font-family',$HashConfi_ref->{"global"}{"font-family"},'font-size',$AAAfontsize);
	}
	else
	{
		$XX3tmp=$XX1-$AAAfontsize*1.1+$ShiftChrNameX;
		my $rotate=$HashConfi_ref->{"global"}{"chr_label_rotation"};
		$svg->text('text-anchor','middle','x',$XX3tmp,'y',$TextYY,'-cdata',$ThisChrName,'font-family',$HashConfi_ref->{"global"}{"font-family"},'font-size',$AAAfontsize,'transform',"rotate($rotate,$XX3tmp,$TextYY)");
	}
...
foreach my $Level  (1..$NumberLevel)
{
	...
	if ( exists $HashConfi_ref->{$Level}{"colormap_brewer_name"}  )
	{
		...
		GetColGradien($ColFlag,$NumGradien,\@ColorGradientArray);
...
################ Dispatch to appropriate plotting function ######################
	...
	if (($HashConfi_ref->{$Level}{"plot_type"}  eq  "heatmap")   ||  ($HashConfi_ref->{$Level}{"plot_type"}  eq  "highlights"))	
	{
        LocalUtils::plot_heatmap($HashConfi_ref,\%ValueToColor, $Level, $svg, \%hashYY1, \%hashYY2, \%hashXX1,
			...
			\@ValueLabelsGradient, $ColorBarSize, $all_chromosomes_spacing, $Bodyheight,$LegendOffsetRatio ,$GradientSteps,$fontsize);
	}
	...
	elsif (( $HashConfi_ref->{$Level}{"plot_type"}  eq  "histogram" )  || ( $HashConfi_ref->{$Level}{"plot_type"}  eq  "hist" ))
	{
    	LocalUtils::plot_histogram($HashConfi_ref,\%ValueToColor, $Level, $svg, \%hashYY1, \%hashYY2, \%hashXX1, 
            $ChrMax,$NumPlotArry, $PlotInfo, $FileData_ref, $FileRow_ref, $ValueToCustomColor_ref,\@ColorGradientArray,
 	        \@ValueLabelsGradient,$ColorBarSize,$all_chromosomes_spacing,$Bodyheight, $LegendOffsetRatio,$GradientSteps,$fontsize,$ChrArry_ref,$hashChr_ref);
	...
	elsif (($HashConfi_ref->{$Level}{"plot_type"}  eq  "scatter" )  || ( $HashConfi_ref->{$Level}{"plot_type"}  eq  "point" ) || ( $HashConfi_ref->{$Level}{"plot_type"}  eq  "points" ))
	{
		LocalUtils::plot_point($HashConfi_ref,\%ValueToColor, $Level, $svg, \%hashYY1, \%hashYY2, \%hashXX1, 
            $ChrMax,$NumPlotArry, $PlotInfo, $FileData_ref, $FileRow_ref, $ValueToCustomColor_ref,\@ColorGradientArray,
 	        \@ValueLabelsGradient,$ColorBarSize,$all_chromosomes_spacing,$Bodyheight, $LegendOffsetRatio,$GradientSteps,
