...
BEGIN {
	use FindBin qw($Bin $RealBin);
	my $ProcessID = $$;  
	my $ProcessCommand = `ps uf -A | awk '\$2==$ProcessID'`;
	my @temp = split /\s+/, $ProcessCommand;

	my $tokenIndex = 12;
	my $CommandPath = $temp[12];
	if ($temp[10] eq "|") {
	} else {
		$tokenIndex = 11;
		$CommandPath = $temp[11];
	...
	if (($CommandPath eq "perl") || ($CommandPath eq "/usr/bin/perl")) {
		$tokenIndex++;
		$CommandPath = $temp[$tokenIndex];
		if (($CommandPath eq "-w") || ($CommandPath eq "-e")) {
			$tokenIndex++;
			$CommandPath = $temp[$tokenIndex];
		}
	}

	$RealBin = `dirname $CommandPath`;
	chomp $RealBin;
}
...
sub usage2 {
	...
	Version:1.39beta         hewm2008\@gmail.com
...
my ($HashConfi_ref, $ValueToCustomColor_ref, $hashChr_ref, $hashChr2File_ref, $ShowColumn_ref,
	...
	$PTypeLink, $ChrMax, $total_chr_length, $MaxGradien, $RegionStart, $bin, $axis_label,
	$ChrCount, $color_gradient_scale, $chr_spacing, $Bodyheight, $fontsize, $height, $width,
	$NumberLevel, $NumParaFlag) = ConfigDataLoader::Deal_Para($InConfi);
...
if ($PTypeLink == 1) {
	...
	my $svg = GenomicLinkPlot::PTypeLinkPlot($HashConfi_ref, $ValueToCustomColor_ref, $hashChr_ref,
		...
		$ChrArry_ref, $ParaFlag_ref, $widthForPerChr, $ChrMax, $total_chr_length, $MaxGradien,
	...
	LocalUtils::svg2PNGfunction($HashConfi_ref, $svg, $OutPut, $Bin, $RealBin);
} else {
	...
	my $svg = RectChrPlot::RectChrPlot($HashConfi_ref, $ValueToCustomColor_ref, $hashChr_ref,
		...
		$ChrMax, $total_chr_length, $MaxGradien, $RegionStart, $bin, $axis_label,
